<?php
	if(isset($_POST['submit'])){
		...
		// echo $disease;
		if ($disease== 'lung'){
			exec("\"E:\\MATLABIIT\\bin\\matlab.exe\" -r \"run('C:\\xampp\\htdocs\\hasura\\M21.m')\"");
		}
		else if($disease== "breast"){
			exec("\"E:\\MATLABIIT\\bin\\matlab.exe\" -r \"run('C:\\xampp\\htdocs\\hasura\\M35.m')\"");
		}
		else if($disease== "collorectal"){
			exec("\"E:\\MATLABIIT\\bin\\matlab.exe\" -r \"run('C:\\xampp\\htdocs\\hasura\\Simple.m')\"");
		}
		else{
			exec("\"E:\\MATLABIIT\\bin\\matlab.exe\" -r \"run('C:\\xampp\\htdocs\\hasura\\M23.m')\"");
		}
		sleep(20);
		// Print Saved results from file
		$t= 20;
		while (!file_exists('C:\\xampp\\htdocs\\hasura\\result.txt')){
			sleep($t);
			$t= $t+10;
		}
		$c= file_get_contents('C:\\xampp\\htdocs\\hasura\\result.txt');
